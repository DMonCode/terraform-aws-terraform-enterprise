resource "aws_security_group" "lb_to_instance" {
  ...
  ingress {
    ...
    from_port   = 22
    to_port     = 22
    cidr_blocks = ["${var.subnets_compute_cidr_blocks}", "${var.subnets_endpoints_cidr_blocks}"]
  }
  ...
  ingress {
    ...
    from_port   = 8800
    to_port     = 8800
    cidr_blocks = ["${var.subnets_endpoints_cidr_blocks}", "${var.subnets_compute_cidr_blocks}"]
  }
}
...
resource "aws_security_group" "lb_public" {
  ...
  egress {
    ...
    from_port   = 0
    to_port     = 0
    cidr_blocks = ["${var.subnets_compute_cidr_blocks}"]
  }
  ...
  egress {
    ...
    from_port   = 0
    to_port     = 0
    cidr_blocks = ["${var.subnets_endpoints_cidr_blocks}"]
  }
}
